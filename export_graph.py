...
Examples:
    ...
    python export_graph.py --graph "C:/Users/dougl/Documents/dfm_scraping/provenance_graph.gpickle" --semantic-edges "C:/Users/dougl/Documents/dfm_scraping/semantic_edges.json" --topic-model "C:/Users/dougl/Documents/dfm_scraping/topic_model_output.json" --output "C:/Users/dougl/My Drive (douglaspmcgowan@gmail.com)/UC Berkeley/Research/Claude Research Folder/dfm-graph-explorer/graph_data.json" -v
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Export DFM graph to graph_data.json for the web explorer.")
    parser.add_argument(
        "--graph",
        type=Path,
        default=Path("C:/Users/dougl/Documents/dfm_scraping/provenance_graph.gpickle"),
        help="path to the gpickle",
    )
    parser.add_argument(
        "--semantic-edges",
        dest="semantic_edges",
        type=Path,
        default=Path("C:/Users/dougl/Documents/dfm_scraping/semantic_edges.json"),
        help="path to semantic_edges.json",
    )
    parser.add_argument(
        "--topic-model",
        dest="topic_model",
        type=Path,
        default=Path("C:/Users/dougl/Documents/dfm_scraping/topic_model_output.json"),
        help="path to topic_model_output.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        ...
        "--verbose",
        action="store_true",
        help="enable per-step progress prints",
    )
...
def main():
    ...
    if args.verbose:
        print(f"Loading semantic edges JSON: {semantic_edges_path}")
    semantic_json = load_json(semantic_edges_path)

    if args.verbose:
        print(f"Loading topic model JSON: {topic_model_path}")
    ...
    if args.verbose:
        print("Collecting node IDs by type...")
    ...
    outlier_count = 0
    if args.verbose:
        print("Processing topic assignments...")
    ...
    semantic_edges_out = []
    if args.verbose:
        print("Assembling semantic edges...")
    ...
    if args.verbose:
        print(f"Writing output JSON: {output_path}")
    ...
    if args.verbose:
        ...
        print(f"Output written: {output_path_abs}")
        print(f"Output size: {output_size} bytes ({output_size_mb:.2f} MB)")

    print(f"Wrote {output_path_abs} | frames={len(frames_out)} | semantic_edges={len(semantic_edges_out)} | topics={len(topics_out)}")
